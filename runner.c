...
void snake_to_camel_test(const char *snake, char *out, size_t outsz) {
    ...
    for (size_t i = 0; snake[i] != '\0' && j + 1 < outsz; ++i) {
        char c = snake[i];
        if (c == '_' || c == '-') { cap_next = 1; continue; }
        if (cap_next) { if (c >= 'a' && c <= 'z') c = c - 'a' + 'A'; cap_next = 0; }
        out[j++] = c;
    }
    const char *suffix = "Test";
    size_t sufflen = strlen(suffix);
    if (j + sufflen + 1 < outsz) {
        strcpy(&out[j], suffix); j += sufflen;
    }
    out[j] = '\0';
}
...
double extract_ops(const char *filename) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (!pos) pos = strstr(line, "'median_ops_per_sec'");
        if (pos) {
            char *colon = strchr(pos, ':');
            if (!colon) continue;
            colon++;
            while (*colon && (*colon==' ' || *colon=='\"' || *colon=='\'' || *colon=='\t' || *colon==',')) colon++;
            val = atof(colon);
            break;
        }
    }
...
int list_files_shell(const char *pattern, char names[][MAX_NAME], int max_names) {
    ...
    while (fgets(buf, sizeof(buf), fp) && count < max_names) {
        ...
        if (strstr(buf, "__pycache__")) continue;
        char base[MAX_NAME];
        extract_basename_noext(buf, base, sizeof(base));
        if (strcmp(base, "runner") == 0) continue;
...
void print_comparison_row(const char *name, double pyops, double rbops) {
    ...
    if (pyops < 0) strcpy(pybuf, "Missing");
    else snprintf(pybuf, sizeof(pybuf), "%.0f", pyops);

    if (rbops < 0) strcpy(rbbuf, "Missing");
    else snprintf(rbbuf, sizeof(rbbuf), "%.0f", rbops);
    ...
    if (pyops > rbops) diff = ((pyops - rbops) / rbops) * 100.0;
    else diff = ((rbops - pyops) / pyops) * 100.0;
...
int main(void) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (threads[thcount]) thcount++;
#else
        if (pthread_create(&threads[thcount], NULL, worker_thread, jpy) == 0) thcount++;
#endif
        ...
        if (threads[thcount]) thcount++;
#else
        if (pthread_create(&threads[thcount], NULL, worker_thread, jrb) == 0) thcount++;
#endif
    }
    ...
    for (int i = 0; i < thcount; ++i) CloseHandle(threads[i]);
#else
    for (int i = 0; i < thcount; ++i) pthread_join(threads[i], NULL);
#endif
